...
********************************************************************************
	...
	*PAGE 2: generating a new caseid which will hold all case data
	...
	gen caseid = 100
	save "$onedrive\Intermediate\tempt_unimp`var'.dta", replace
	restore
	}
	...
	foreach var of local analysis {	
	use "$onedrive\Intermediate\tempt_unimp`var'.dta", clear
	collapse (max) `var'_cpr hh_actc`var' totalctime_`var' caseid, by(`var')
	save "$onedrive\Intermediate\tempt_unimp`var'.dta", replace
	use "$onedrive\Intermediate\tempa_unimp.dta", clear
	append using "$onedrive\Intermediate\tempt_unimp`var'.dta"
	save "$onedrive\Intermediate\tempa_unimp.dta", replace
	}
	...
	*Generating case name variable
	...
	label 	define 	casenmelbl 			1 	"Dan Rogger"				 		///
										2 	"Kerenssa M. Kay"				 	///
										3	"Leyla Castillo"					///
										4	"Sushmita Samaddar"					///
										5	"Vincenzo Di Maro"					///
	...
	label 	values	casenme				casenmelbl

	save "$onedrive\Intermediate\tud_11182019_v3_unimp.dta", replace
********************************************************************************
...
********************************************************************************
	...
	*PAGE 2: generating a new caseid which will hold all case data
	...
	drop if `var'_cpr ==.	| `var' == . | hh_actc`var' ==. | totalctime_`var'==.
	gen caseid = 100
	save "$onedrive\Intermediate\tempt_imp`var'.dta", replace
	restore
	}
	...
	foreach var of local analysis {	
	use "$onedrive\Intermediate\tempt_imp`var'.dta", clear
	collapse (max) `var'_cpr hh_actc`var' totalctime_`var' caseid, by(`var')
	save "$onedrive\Intermediate\tempt_imp`var'.dta", replace
	use "$onedrive\Intermediate\tempa_imp.dta", clear
	append using "$onedrive\Intermediate\tempt_imp`var'.dta"
	save "$onedrive\Intermediate\tempa_imp.dta", replace
	}
	...
	*Generating case name variable
	...
	save "$onedrive\Intermediate\tud_11182019_v3_imp.dta", replace
